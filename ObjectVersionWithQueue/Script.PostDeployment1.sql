...
if exists (select * from sys.messages where message_id = 50073) 
EXEC sp_dropmessage  @msgnum = 50073, @lang = 'us_english' 

EXEC sp_addmessage @msgnum = 50073, @severity = 16,   
@msgtext = N'Error 50073. Message from host [%s] rejected. USER=[%s]. HOST=[%s]. 
Please check the state of the table [DBCodes].',
   @lang ='us_english', @with_log = true;  
GO 

if exists (select * from sys.messages where message_id = 50074) 
EXEC sp_dropmessage  @msgnum = 50074, @lang = 'us_english' 

EXEC sp_addmessage @msgnum = 50074, @severity = 16,   
@msgtext = N'Error 50074. Object [%s] rejected.
Please check the state of the table [BlackObjectList].',
   @lang ='us_english', @with_log = false;  
