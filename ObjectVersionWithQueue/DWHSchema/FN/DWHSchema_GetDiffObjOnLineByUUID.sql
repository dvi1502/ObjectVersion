...
CREATE FUNCTION [DWHSchema].[GetDiffObjOnLineByUUID]
(
	@UUID		varchar(36),
	@CurCh		bigint,
	@PrevCh		bigint
...
BEGIN
	...
	WHERE [curobj_checksum] = @CurCh and [refuuid] = @UUID

	SELECT TOP(1) @PrevObj = CONVERT(xml,DECOMPRESS(s.[curobj]))
	FROM [DWHSchema].[VersionsObjectStorage] s
	WHERE [curobj_checksum] = @PrevCh and [refuuid] = @UUID
	...
	(
		SELECT null, null, [key] as [Key], MAX(v0) as [NewVal], MAX(v1) as [OldVal] 
		FROM 
		(
...
GRANT SELECT ON [DWHSchema].[GetDiffObjOnLineByUUID]
